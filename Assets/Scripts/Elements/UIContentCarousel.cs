...
public interface IAssociatedTextHandler
{
...
{
    public enum UPDATE_ASSOCIATED_TEXT_MODE
    {
        USE_IASSOCIATED_TEXT_HANDLER,
        USE_STRINGS_ARRAY
    }
    ...
    [Header("Text Associated with Images")]
    public UPDATE_ASSOCIATED_TEXT_MODE updateAssociatedTextMode = UPDATE_ASSOCIATED_TEXT_MODE.USE_STRINGS_ARRAY;
    public string[] imageTexts; // Array to hold text corresponding to each image
    ...
    private IAssociatedTextHandler associatedTextHandler;

    public void SetAssociatedTextHandler(IAssociatedTextHandler associatedTextHandler)
    {
        this.associatedTextHandler = associatedTextHandler;
    }


    private void UpdateAssociatedText()
    {
        switch (updateAssociatedTextMode)
        {
            case UPDATE_ASSOCIATED_TEXT_MODE.USE_IASSOCIATED_TEXT_HANDLER:
                associatedTextHandler.UpdateAssociatedText(currentIndex);
                break;

            case UPDATE_ASSOCIATED_TEXT_MODE.USE_STRINGS_ARRAY:
                if (associatedText != null && imageTexts.Length > 0)
                {
                    associatedText.text = imageTexts[currentIndex];
                }
                break;
        }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (carouselMode)
        {
            ...
            if (dragDistance > pageSize * swipeThreshold || Mathf.Abs(dragSpeed) > swipeThreshold)
            {
                ...
                //if (dragSpeed > 0)
                if ( eventData.position.x - dragStartPos.x > 0 )
                {
                    MoveToPreviousPage();
                }
                else
